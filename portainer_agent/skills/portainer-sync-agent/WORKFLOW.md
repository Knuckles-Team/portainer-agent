...
Portainer Sync Agent atomic skill. Connects to Portainer API, resolves environment IDs, creates or redeploys stacks, and wires GitOps auto-sync configurations using portainer-mcp.
...
- Set repository URL (e.g. `[configured-endpoint]`).
...
```bash
PORTAINER_URL=[configured-endpoint] PORTAINER_TOKEN=… \
...
line; `--set KEY=VALUE` is available for non-secret vars. Uses the mandatory-verification TLS profile resolved from XDG AgentConfig, including runtime CA and mTLS references. Find the stack id via the Portainer stacks
