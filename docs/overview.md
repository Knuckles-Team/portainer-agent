# portainer-agent — Concept Overview
...
```json
{
  "mcpServers": {
    "portainer-agent": {
      "command": "uv",
      "args": ["run", "--with", "portainer-agent", "portainer-mcp"],
      "env": {}
    }
  }
}
...
```bash
portainer-mcp --transport streamable-http --port 8001
```
